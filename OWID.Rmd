---
title: "OWID Covid data visualization"
author: "Kyle Deng"
...
## Graph 1: Covid cases vs Vaccinations in Canada
```{r}
...
## Graph 2: Covid vaccination progress in Canada
```{r}
...
#           fillcolor = ~doses, line = list(width = 0.5)))
```


```{r}
# plot_ly(CA, x = ~date, y = ~total_cases, type = "scatter", mode = "lines")
# 
# CA %>%
#   drop_na(new_cases_smoothed) %>%
#   mutate(positive_rate = positive_rate * 100) %>%
#   ggplot(aes(x = date)) +
#     geom_line(aes(y = positive_rate, color = "blue"), size = 1) +
#     geom_line(aes(y = stringency_index, color = "red"), size = 1) +
